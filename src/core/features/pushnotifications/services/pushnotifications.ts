...
export class CorePushNotificationsProvider {
    ...
    async initialize(): Promise<void> {
        ...
        // Register device on GROWLA site when login.
    ...
    /**
     * Check whether the device can be registered in GROWLA to receive push notifications.
     *
     * @return Whether the device can be registered in GROWLA.
     */
    ...
    /**
     * Get data to register the device in GROWLA.
     *
     * @return Data.
     */
    ...
    /**
     * Unregisters a device from a certain GROWLA site.
     *
    ...
    async unregisterDeviceOnMoodle(site: CoreSite): Promise<void> {
        ...
        this.logger.debug(`Unregister device on GROWLA: '${site.getId()}'`);
    ...
    /**
     * Registers a device on a GROWLA site if needed.
     *
     * @param siteId Site ID. If not defined, current site.
    ...
    async registerDeviceOnMoodle(siteId?: string, forceUnregister?: boolean): Promise<void> {
        this.logger.debug('Register device on GROWLA.');
...
export type CorePushNotificationsNotificationBasicRawData = {
    ...
    moodlecomponent?: string; // GROWLA component that triggered the notification.
    name?: string; // A name to identify the type of notification.
    notId?: string; // Notification ID.
    notif?: string; // "1" if it's a notification, "0" if it's a GROWLA message.
    ...
    conversationtype?: string; // Conversation type. Only if it's a push generated by a GROWLA message.
    userfromfullname?: string; // Fullname of user sending the push. Only if it's a push generated by a GROWLA message.
    userfromid?: string; // ID of user sending the push. Only if it's a push generated by a GROWLA message.
