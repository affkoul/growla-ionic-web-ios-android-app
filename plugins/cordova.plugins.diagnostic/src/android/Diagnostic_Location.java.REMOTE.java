...
public class Diagnostic_Location extends CordovaPlugin{
    ...
    public void requestLocationAuthorization(JSONArray args, CallbackContext callbackContext) throws Exception{
        ...
        if(specifiedPermissions != null){
            for (int i = 0; i < specifiedPermissions.length(); i++) {
                String specifiedPermission = specifiedPermissions.getString(i);
                if(specifiedPermission.equals(gpsLocationPermission) ){
                    permissionsToRequest.put(gpsLocationPermission);
                }else if(specifiedPermission.equals(networkLocationPermission)){
                    permissionsToRequest.put(networkLocationPermission);
                }else if(specifiedPermission.equals(backgroundLocationPermission) && Build.VERSION.SDK_INT >= 29 ){
